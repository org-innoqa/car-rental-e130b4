...
export default function Fleet({ vehicles, onRequest }: FleetProps) {
  ...
  const selectedVariant = (vehicle: FleetVehicle) => {
    ...
    return variants.find(item => item.name === choices[modelName(vehicle)]) || variants.find(item => modeOf(item) === 'Chauffeur service') || variants[0] || vehicle;
  };
  ...
  return <main>
    ...
    <section className="border-y border-line bg-ink text-white"><div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 sm:grid-cols-3 lg:px-8 lg:py-16"><FleetPromise title="Chauffeur included" text="Selected models are also available with a professional, discreet chauffeur." /><FleetPromise title="Reviewed before payment" text="Your chosen vehicle is requested first. Payment follows availability approval." /><FleetPromise title="Concierge guidance" text="Not sure what suits your route? Our team can recommend the right option." /></div></section>
