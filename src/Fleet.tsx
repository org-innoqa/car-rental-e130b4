...
export default function Fleet({ vehicles, onRequest }: FleetProps) {
  ...
  const visibleVehicles = useMemo(() => {
    const filtered = vehicles.filter(vehicle => {
      ...
      const matchesOccasion =
        ...
        (occasion === 'VIP arrivals' && vehicle.category.toLowerCase().includes('executive'));

      return matchesCategory && matchesBrand && matchesOccasion;
    });
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sort === 'Most Popular') return Number(b.badge.includes('choice')) - Number(a.badge.includes('choice'));
      return a.price - b.price;
    });
  }, [vehicles, category, brand, occasion, sort]);

  const toggleCompare = (vehicle: FleetVehicle) => {
    setCompare(current => {
      if (current.some(item => item.name === vehicle.name)) {
        return current.filter(item => item.name !== vehicle.name);
      }
      return current.length < 3 ? [...current, vehicle] : current;
    });
  };
  ...
  return (
    <main>
        ...
        <div className="mx-auto max-w-7xl px-5 pb-14 pt-12 lg:px-8 lg:pb-20 lg:pt-20">
          <div className="grid gap-10 lg:grid-cols-[1.1fr_.7fr] lg:items-end">
            <div>
              ...
              <h1 className="mt-5 max-w-3xl font-display text-5xl font-semibold leading-[1.03] tracking-[-.05em] sm:text-7xl">
                Find the right vehicle for your <em className="font-normal text-stone-500">journey.</em>
              </h1>
            </div>
            <div>
              <p className="max-w-md text-base leading-7 text-stone-500">
                Premium sedans, SUVs and spacious vans, each arranged with a professional chauffeur across Qatar.
              </p>
              <div className="mt-6 flex flex-wrap gap-3 text-xs text-stone-500">
                <span className="rounded-full border border-line px-3 py-2">Chauffeur included</span>
                <span className="rounded-full border border-line px-3 py-2">Approval before payment</span>
              </div>
            </div>
          </div>

          <div className="mt-12 grid gap-5 rounded-2xl bg-mist p-5 sm:grid-cols-3 sm:p-6">
            <FleetStat value={`${vehicles.length}`} label="Premium vehicles" />
            <FleetStat value="24/7" label="Concierge availability" />
            <FleetStat value={vehicles.length ? `QAR ${Math.min(...vehicles.map(vehicle => vehicle.price)).toLocaleString()}` : 'QAR —'} label="Estimated journeys from" />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-16">
        <div className="border-b border-line pb-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 font-display text-3xl font-semibold tracking-tight">Choose by occasion</h2>
            </div>
            <p className="max-w-sm text-sm leading-6 text-stone-500 lg:text-right">
              Compare up to three vehicles before requesting a quote from our concierge team.
            </p>
          </div>

          <div className="mt-7 flex flex-wrap gap-3">
            <FilterSelect value={occasion} onChange={setOccasion} options={occasions} />
            ...
            <FilterSelect
              ...
              options={['Recommended', 'Most Popular', 'Price: Low to High', 'Price: High to Low', 'Passenger Capacity']}
            />
          </div>
        </div>
          ...
          {visibleVehicles.map(vehicle => {
            ...
            return (
              <article key={vehicle.name} className="group overflow-hidden rounded-2xl border border-line bg-white transition-shadow hover:shadow-xl">
                <div className="relative h-64 overflow-hidden bg-[#f7f7f4]">
                  <img
                    src={vehicle.image}
                    alt={`${vehicle.name} luxury chauffeur vehicle`}
                    className="h-full w-full object-contain mix-blend-multiply transition-transform duration-500 group-hover:scale-105"
                  />
                  <span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[.1em] text-[#876b42] shadow-sm">
                    {vehicle.badge}
                  </span>
                  <button
                    type="button"
                    onClick={() => toggleCompare(vehicle)}
                    className={`absolute right-4 top-4 flex items-center gap-2 rounded-full px-3 py-2 text-xs font-semibold shadow-sm ${isCompared ? 'bg-ink text-white' : 'bg-white text-ink'}`}
                  >
                ...
                <div className="p-5">
                  ...
                  <div className="mt-5 flex items-end justify-between gap-3">
                    ...
                    <button type="button" onClick={() => setDetails(vehicle)} className="text-xs font-semibold underline underline-offset-4">
                      View details
                    </button>
                  </div>

                  <button type="button" onClick={() => onRequest(vehicle)} className="mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-ink px-4 py-3 text-sm font-semibold text-white">
        ...
        {!visibleVehicles.length && (
          <div className="rounded-2xl border border-line bg-white px-6 py-16 text-center">
            <h2 className="font-display text-2xl font-semibold">No vehicles match those filters</h2>
            <button type="button" onClick={clearFilters} className="mt-4 text-sm font-semibold underline">Clear filters</button>
          </div>
        )}
      </section>
      ...
      {compare.length > 0 && <Comparison vehicles={compare} onRemove={toggleCompare} onClose={() => setCompare([])} onRequest={onRequest} />}
      {details && <VehicleDetails vehicle={details} onClose={() => setDetails(null)} onRequest={onRequest} />}
    </main>
  );
}

function FleetStat({ value, label }: { value: string; label: string }) {
  return <div><p className="font-display text-2xl font-semibold">{value}</p><p className="mt-1 text-xs uppercase tracking-[.12em] text-stone-500">{label}</p></div>;
}

function FleetPromise({ title, text }: { title: string; text: string }) {
  return <div><Check className="text-sand" size={21} /><h3 className="mt-4 font-display text-xl font-semibold">{title}</h3><p className="mt-2 max-w-xs text-sm leading-6 text-stone-400">{text}</p></div>;
}
